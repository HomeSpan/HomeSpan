...
* **O** - prompts you to set the password used for *optional* Over-the-Air (OTA) Updates
  * HomeSpan supports [OTA Updates](OTA.md) but requires a password to always used.  For security, HomeSpan saves a *hashed* version of the password you specify in NVS, not the actual password.  If you forget the password you specified, you'll need to reset it using this command.
  * If left unspecified, HomeSpan uses a "homespan-ota" as its default OTA password. 
