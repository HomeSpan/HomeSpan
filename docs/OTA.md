...
In this fashion, if you upload a new sketch into your device via OTA and the new sketch crashes immediately upon rebooting, rather than simple repeating an endless cycle of booting and crashing, the operating system can instead rollback to the original working sketch when it restarts after the first crash.  Once the original sketch is running again, you will be able to repair your new sketch and upload again via OTA.  This is a terrific safety mechanism and helps ensure you don't accidental render a remote device inoperable by uploading a flawed sketch that crashes immediately and prevents additional OTA uploads.

By default, the Arduino-ESP32 library automatically re-marks any partition booted with an OTA State of *PENDING_VERIFY* to *VALID* during initialization and **before** the Arduino `setup()` function  is called.  In other words, the default behavior of the Arduino-ESP32 library is to automaticalluy self-validate every OTA sketch upon boot-up before the user's sketch even has a chance to perform any self-tests.  As a result, the safety of the rollback mechanism is rendered ineffective as the same partition will be run again and again even if it panics and crashes immediately after calling the Arduino `setup()` function.

Fortunately, the Arduino-ESP32 library contains a hook that allows a sketch to disable this default behavior and prevents the Arduino-ESP32 library from automatically re-marking every partition with OTA State of *PENDING_VERIFY* to *VALID*.  However, because this auto-validation occurs before the Arduino `setup()` function is called, it cannot be disabled by calling a "disable" method from within the `setup()` function --- it is too late by that point.  Instead, to disable auto-validation by the Arduino-ESP32, you must add the following to the top of your sketch:

```C++
#include "SpanRollback.h"
```

By including this header file in your sketch, the Arduino-ESP32 library will NOT automatically validate sketches uploaded via OTA, and the OTA State of the partition will instead remain marked as *PENDING_VERIFY* until the sketch itself re-marks the partition as *VALID* prior to the next reboot (or crash).

> [!Note]
> When this header is included, it is the users responsibility to re-mark the OTA State of the partition as *VALID* somewhere within the sketch.  If not, then upon rebooting (either purposefully or as the result of a panic), the bootloader will rollback to a prior sketch in a previously-validated partition. If the sketch you've just uploaded and has a bug that caused it to panic and crash early on, the resulting rollback is beneficial and will allow you to fix your issue and re-upload a new sketch.  But if you simply forgot to re-mark a working sketch as *VALID* and at some point in the future the device reboots, you will be surprised to find the device has reverted to prior version of your sketch!
