...
The ESP32 operating system marks each OTA APP partition in a partition table with one of the following OTA States: *NEW*, *PENDING_VERIFY*, *VALID*, *INVALID*, *ABORTED*, or *UNDEFINED*.  The OTA State of each OTA App partition determines whether or not that partition is selectable by the bootloader for running when the device is rebooted.[^undefined]  Whenever a new sketch is successfully uploaded via OTA, the operating system marks the partition into which the sketch was uploaded as *NEW*, flags it to be run upon start-up, and then (typically) reboots the device.

[^undefined]: Sketches uploaded via USB are generally placed into the first APP partition and the operating system automatically marks the OTA State as UNDEFINED upon completion of the upload.  An APP partition marked as UNDEFINED is always selectable for booting at start-up.

Upon rebooting, just before the new sketch starts to run, the operating system re-marks the OTA State of the partition from *NEW* to *PENDING_VERIFY*.  When a sketch is running from a partition marked as *PENDING_VERIFY*, the operating system is expecting that sometime **before the next reboot of the device**, the sketch will itself re-mark the currently-running partition as *VALID*.  If the device is rebooted before the partition is re-marked as VALID, then upon rebooting the operating system will re-mark the OTA State of the partition to *ABORTED*, the partition will no longer be bootable, and instead the bootloader will "rollback" to running whatever sketch is stored in the last known *VALID* partition.

In this fashion, if you upload a new sketch into your device via OTA and the new sketch crashes immediately upon rebooting, rather than simple repeating an endless cycle of booting and crashing, the operating system can instead rollback to the original working sketch when it restarts after the first crash.  Once the original sketch is running, you will once again be able to upload a new (and hopefully corrected) sketch via OTA and try again. This is a terrific safety mechanism and helps ensure you don't accidental render a remote device inoperable by uploading a flawed sketch that crashes immediately and prevents additional OTA uploads.
...
Fortunately, the Arduino-ESP32 library also contains a method to *prevent* the library from automatically re-marking the OTA State of a *PENDING_VERIFY* partition to *VALID*.  To implement this method in HomeSpan simply add the following to the top of your sketch:

```C++
#include "SpanRollback.h"
```

By including this header file in your sketch, the Arduino-ESP32 library will NOT automatically validate sketches uploaded via OTA, and the OTA State of the partition will instead remain marked as *PENDING_VERIFY* until the sketch itself re-marks the partition as *VALID* prior to the next reboot (or crash).  To mark the partition *VALID* from within your sketch, call the homeSpan method `markSketchOK()`. 

> [!Caution]
> When this header is included, it is the users responsibility to re-mark the OTA State of the partition as *VALID* somewhere within the sketch by calling `homeSpan.markSketchOK()`.  If not, then upon rebooting (either purposefully or as the result of a panic), the bootloader will rollback to a prior sketch in a previously-validated partition. If the sketch you've just uploaded and has a bug that caused it to panic and crash early on, the resulting rollback is beneficial and will allow you to fix your issue and re-upload a new sketch.  But if you simply forgot to re-mark a working sketch as *VALID* and at some point in the future the device reboots, you will be surprised to find the device has reverted to prior version of your sketch[^invalid].

[^invalid]: A sketch can also proactively re-mark the OTA State of the currently-running partition to *INVALID* and immediately reboot the device.  Similar to partitions marked *ABORTED*, partitions marked *INVALID* are not selectable for booting and upon startup the bootloader will instead rollback to running whatever sketch is stored in the last known *VALID* partition.  However, marking a partition as *INVALID* from within a sketch is uncommon since it the sketch has critical failures it will likely crash and reboot before it is marked *VALID* and the operating system will re-mark the partition as *ABORTED*.

When using SpanRollback, one question you must answer is where in your sketch to perform the validation.  If you call `homeSpan.markSketchOK()` too early (i.e. near the top of the `setup()` function) than you may have validated your sketch before it had a chance to run through all the setup functionality and you won't be able to protect against any crashes or panics that may occur in those sections.

One possible choice is to validate your sketch only once WiFi connectivity or Ethernet connectivity has been established, perhaps from within a callback you create using `homeSpan.setConnectionCallback().`  Since this callback does not occur until WiFi connectivity has been established, this necesssarily means all the setup functions and HomeSpan initializations have occured and the HomeSpan `poll()` task has run a number of times --- if something was going to crash, it probably would have already.  However, if your device has any problems connecting to your WiFi network after rebooting, and the problem is with the network (not the sketch) such that you have to reboot your device, then it will rollback to a previous version since the sketch was not yet validated.

To avoid this issue another possible choice is to validate your sketch once the HomeSpan `poll()` function has been the very first time, perhaps from with a callback you create using, `homeSpan.setPollingCallback().` This callback occurs late enough to allow all the setup functions and HomeSpan initializations to have run, as well as ensure a first pass through the `poll()` task has been successful, but without needing to wait for WiFi connectivity to be established.

Every sketch is different so the choice of where and when to validate, as well as whether to peform any sketch-specific validations tests,[^validation] are up to your own discretion and preferences.

[^validation]: The primary purpose of enabling OTA Rollbacks in HomeSpan is to protect against complete failures in a new sketch (such as an endless crash/reboot cycle) that prevents you from uploading a corrected sketch via OTA, and forces you to instead retrive the device and upload the correct sketch via USB.  Though you can create additional "validation tests" that the sketch must pass before deciding to mark it *VALID*, even if the sketch is stable and does not crash (i.e. OTA uploading is still possible), these type of bugs and issues are usually easier to detect during normal operations, and can be addressed by uploading corrected sketches via OTA as needed.
